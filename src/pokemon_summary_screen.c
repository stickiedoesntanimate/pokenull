...
static void DrawExperienceProgressBar(struct Pokemon *unused)
{
    ...
    if (summary->level < MAX_LEVEL)
    {
        ...
        // There are 8 tiles that make up the bar, and eac/((h tile has 8 "ticks". Hence, the numerator
        // is multiplied by 64.
    ...
    dst = &sMonSummaryScreen->bgTilemapBuffers[PSS_PAGE_SKILLS][1][0x195];
    for (i = 0; i < 8; i++)
    {
...
void BufferStat(u8 *dst, u8 statIndex, u32 stat, u32 strId, u32 n)
{
...
static void PrintNewMoveDetailsOrCancelText(void)
{
    ...
    else
    {
        ...
        DynamicPlaceholderTextUtil_ExpandPlaceholders(gStringVar4, sMovesPPLayout);
        PrintTextOnWindow(windowId2, gStringVar4, GetStringRightAlignXOffset(FONT_NORMAL, gStringVar4, 44), 65, 0, 5);
    }
}
